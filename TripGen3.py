# This file contains the iteration in which a department is developed, generating attraction, taking some pressure out of Paris
import numpy as np
import pandas as pd
import random
# importing Data
...
#assuming only 80% of people living in Paris work in Paris, 10% to Hauts de Seine and the rest to 94 and 93, more to 94
#zone 75
frac_75 = 0.28
work_75 = np.array(["75", "92", "94", "93"])
p_75 = (0.8, 0.1, 0.07, 0.03)
for i in range(len(((df3["zone"] == "75") & ((df3["following_purpose"] == "work") | (df3["preceding_purpose"] == "work"))))):
    a = np.random.choice(work_75, replace=False, p=p_75)
    if (df3.loc[i, "zone"] == "75") & ((df3.loc[i, "following_purpose"] == "work") | (df3.loc[i, "preceding_purpose"] == "work")):
        df3.loc[i, "workplace"] = a

#zone 92
frac_92 = 0.28
work_92 = np.array(["75", "92", "94", "93"])
p_92 = (0.2, 0.7, 0.07, 0.03)
for i in range(len(((df3["zone"] == "92") & ((df3["following_purpose"] == "work") | (df3["preceding_purpose"] == "work"))))):
    a = np.random.choice(work_92, replace=False, p=p_92)
    if (df3.loc[i, "zone"] == "92") & ((df3.loc[i, "following_purpose"] == "work") | (df3.loc[i, "preceding_purpose"] == "work")):
        df3.loc[i, "workplace"] = a

#decrease amount of people staying in 93 to 62% and distribute the rest in the same way as before
#zone 93
frac_93 = 0.28
work_93 = np.array(["75", "92", "94", "93"])
p_93 = ((1-0.62)*0.5, (1-0.62)*0.3, (1-0.62)*0.2, 0.62)
...
p_94 = ((1-0.66)*0.5, (1-0.66)*0.3, (1-0.66)*0.2, 0.66)
